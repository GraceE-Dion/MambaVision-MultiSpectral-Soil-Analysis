...
=========================
Runs inference on laser crop test images using the best
MambaVision_S + FFT + Wavelet model.
Samples 50 images across 7 source datasets.
Produces annotated images in the same format as 07_inference_pipeline.py.
...
Run:
    python 07c_inference_pipeline.py

Output:
    results/inference_images_fft_wavelet/   — annotated images
    results/inference_summary_fft_wavelet.json
"""
...
# ── Paths ─────────────────────────────────────────────────────────────────────
RESULTS_DIR = "./results"
OUTPUT_DIR  = "./results/inference_images_fft_wavelet"
MODEL_PATH  = "./results/mambavision_fft_wavelet_best_model.pth"
...
def build_7channel_tensor(img_pil, transform):
    ...
    tensor  = transform(img_pil)                          # (3, H, W)
    ...
    return torch.cat([tensor, fft_tensor, wav_tensor], dim=0)  # (7, H, W)

# ═════════════════════════════════════════════════════════════════════════════
# 4. MODEL — MambaVisionFFTWavelet wrapper (same as 05c)
# ═════════════════════════════════════════════════════════════════════════════
...
print("Loading MambaVision_S + FFT/Wavelet laser crops model...")
...
summary = {
    "model"            : "MambaVision_S_FFTWavelet_LaserCrops",
...
summary_path = os.path.join(RESULTS_DIR, "inference_summary_fft_wavelet.json")
with open(summary_path, "w") as f:
    json.dump(summary, f, indent=2)

print(f"\nSummary saved → {summary_path}")
print(f"Images saved  → {OUTPUT_DIR}")
print("\nDone! Ready for 07bc_inference_pipeline.py")