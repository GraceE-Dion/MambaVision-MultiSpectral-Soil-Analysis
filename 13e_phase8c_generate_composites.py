...
def generate_candidate_offsets_dense(region_w, region_h,
                                      radii=None, n_angles=24):
    """Genuinely dense deterministic search grid: a full radius x angle
    sweep (not a fixed set of 8 compass points). Per peer review's
    Finding 2 -- the prior "8 directions x a few fixed radii" design
    was found too sparse and prone to silently falling through to
    worse tiers on edge-adjacent boxes; this replaces it with a real
    expansion, not a cosmetic 4->8 directions tweak.

    Offsets are generated by rotating a unit vector through n_angles
    steps around the full circle at each of several radius multipliers
    (of the region's own width/height, so ellipse-shaped rather than
    forcing exact compass alignment), then rounded to int pixel
    offsets and deduplicated. Returns offsets sorted by radius
    (nearest first) purely for readability of diagnostics; every
    offset is still scored and the global best is selected regardless
    of order.
    """
    if radii is None:
        radii = [0.3, 0.6, 0.9, 1.2, 1.5, 1.8, 2.1, 2.4, 2.7, 3.0]
    seen = set()
    offsets = []
    for r in radii:
        for i in range(n_angles):
            theta = 2 * np.pi * i / n_angles
            dx = int(round(r * region_w * np.cos(theta)))
            dy = int(round(r * region_h * np.sin(theta)))
            if dx == 0 and dy == 0:
                continue
            key = (dx, dy)
            if key in seen:
                continue
            seen.add(key)
            offsets.append((r, dx, dy))
    offsets.sort(key=lambda t: t[0])
    return [(dx, dy) for _, dx, dy in offsets]


def _search_candidates(donor_img, px_min, py_min, px_max, py_max,
                        region_w, region_h, surround_stats):
    """Run the dense offset search once for a given region and return
    every geometrically valid, non-overlapping, in-bounds candidate
    patch with its similarity score. Shared by tier 1 and tier 2 (tier
    2 just calls this again on a shrunk region)."""
    img_h, img_w = donor_img.shape[:2]
    candidates = []
    for dx, dy in generate_candidate_offsets_dense(region_w, region_h):
...
def repair_donor_roi(donor_img, donor_bbox, pad_fraction=0.6, min_pad_px=15):
    ...
    Tier structure (Finding 2 redesign -- every repair, regardless of
    tier, gets a populated `tier` field (1/2/3) and a populated
    `selected_score`, so tier can be used as a real queryable metadata
    field in Phase 8E sensitivity analysis, and no repair is left with
    a None score that would silently exclude it from score-based QA
    flags):

    Tier 1: dense radius x angle scored search at the full pad region.
    Tier 2: if tier 1 finds literally zero valid candidates (region
        too large or too close to an image edge), progressively SHRINK
        the padded region itself (not just the search radii) and
        re-run the same dense search -- a smaller region is
        geometrically more likely to fit somewhere in-bounds. This
        replaces the old "same region, smaller radii" retry, which
        could still fail outright when the region itself didn't fit.
    Tier 3 (last resort): no candidate fits at any shrink level --
        take the best-scoring adjacent block of available space
        (top/bottom/left/right, scored by appearance not just size)
        and resize it to fit; if no adjacent space exists at all, flat
        fill from the surround color. Both sub-cases get a real score
        computed against the same surround stats used everywhere else.
    ...
    def _region_for_pad(pf, min_pad):
        pad_x = max(min_pad, int(box_w * pf))
        pad_y = max(min_pad, int(box_h * pf))
        pmin_x = max(0, x_min - pad_x)
        pmin_y = max(0, y_min - pad_y)
        pmax_x = min(img_w, x_max + pad_x)
        pmax_y = min(img_h, y_max + pad_y)
        return pad_x, pad_y, pmin_x, pmin_y, pmax_x, pmax_y

    pad_x, pad_y, px_min, py_min, px_max, py_max = _region_for_pad(pad_fraction, min_pad_px)
    region_w = px_max - px_min
    region_h = py_max - py_min
    surround_stats = get_surround_stats(donor_img, px_min, py_min, px_max, py_max)

    # TIER 1
    candidates = _search_candidates(donor_img, px_min, py_min, px_max, py_max,
                                     region_w, region_h, surround_stats)
    tier = 1
    shrink_fraction_used = None

    # TIER 2: shrink the padded region itself (not just the search
    # radii) and re-run the dense search at each shrink level, largest
    # first, stopping at the first level that yields >=1 candidate.
    if not candidates:
        tier = 2
        for shrink_pf, shrink_min_pad in [(0.40, 10), (0.25, 6), (0.15, 4), (0.08, 2)]:
            pad_x, pad_y, px_min, py_min, px_max, py_max = _region_for_pad(shrink_pf, shrink_min_pad)
            region_w = px_max - px_min
            region_h = py_max - py_min
            surround_stats = get_surround_stats(donor_img, px_min, py_min, px_max, py_max)
            candidates = _search_candidates(donor_img, px_min, py_min, px_max, py_max,
                                              region_w, region_h, surround_stats)
            shrink_fraction_used = shrink_pf
            if candidates:
                break
    ...
    diagnostic = {
        ...
        # Default; overridden to False only by the tier3 no-texture
        # constant-fill sub-case below (peer review: a numeric score
        # alone does not make that pathway a legitimate repair outcome).
    ...
    if not candidates:
        ...
        if scored_tier3:
            ...
            tier3_score = best3["score"]
            method = "tier3_adjacent_space_resize"
        else:
            # Genuinely no adjacent space at all -- constant-color fill
            # from surround statistics as an absolute last resort;
            # should be exceptionally rare. Still scored against the
            # same surround stats, per Finding 2: no repair may leave
            # selected_score as None, since that silently drops it
            # from any score-based QA flagging downstream.
            #
            # Per peer review on the redesign: this pathway is kept
            # for robustness (so the pipeline never crashes on a
            # pathological donor) but is NOT treated as a legitimate
            # repair outcome -- we previously rejected flat/constant
            # blocks precisely because they introduce artificial
            # features. A numerical similarity score does not make
            # this admissible; repair_admissible is set False and this
            # composite must not silently enter the B/C experimental
            # set without a human decision.
            ...
            tier3_score = patch_similarity_score(reflected, surround_stats)
            method = "tier3_no_texture_fallback"
        ...
        diagnostic.update({"method": method,
                            "selected_score": tier3_score, "selected_source": None,
                            "repair_admissible": (method != "tier3_no_texture_fallback")})
    ...
    diagnostic.update({
        "method": "tier1_dense_search" if diagnostic["tier"] == 1 else "tier2_shrunk_region_search",
...
def save_diagnostic_panel(donor_img, donor_bbox, target_img, target_bbox,
                           diagnostic, composite, repaired_donor, out_path):
    ...
    Takes the already-computed repaired_donor image (from the shared
    per-run cache) rather than recomputing it, so the panel always
    shows exactly the same repaired background that was saved as its
    own standalone artifact and used in the actual composite -- no
    risk of the QA panel silently drifting from what was really used."""
...
def composite_image(target, donor, donor_repair_cache, repaired_donor_dir=None):
    ...
    The donor repair depends only on the donor image + its own bbox,
    not on which target/condition it's being composited into, so it
    is computed and saved ONCE per donor and reused across every
    composite that draws on that donor (Finding 2, point (d)): saving
    the repaired background as its own standalone artifact lets a
    repair defect be distinguished from a compositing defect without
    regenerating anything, and caching avoids paying the dense search
    cost (and 3x'ing disk usage) redundantly across A/B/C x donors.
    """
    ...
    if donor["stem"] in donor_repair_cache:
        donor_clean, diagnostic = donor_repair_cache[donor["stem"]]
    else:
        donor_clean, diagnostic = repair_donor_roi(donor_img, donor["bbox"])
        donor_repair_cache[donor["stem"]] = (donor_clean, diagnostic)
        if repaired_donor_dir is not None:
            os.makedirs(repaired_donor_dir, exist_ok=True)
            repaired_path = os.path.join(repaired_donor_dir, f"{donor['stem']}__repaired_donor.png")
            cv2.imwrite(repaired_path, donor_clean)
            diagnostic["repaired_donor_path"] = repaired_path
...
def main():
    ...
    repaired_donor_dir = os.path.join(out_dir, "repaired_donors")

    # Keyed by donor stem so a donor's repair is computed and saved
    # exactly once per run, then reused across every composite that
    # draws on it (Finding 2, point (d) -- see composite_image()).
    donor_repair_cache = {}
        ...
        for condition in ["A", "B", "C"]:
            ...
            for donor_idx, donor in enumerate(donors):
                composite, diagnostic, roi_ok, roi_max_diff = composite_image(
                    target, donor, donor_repair_cache, repaired_donor_dir)
                if not roi_ok:
                ...
                if not args.full:
                    ...
                    repaired_donor_cached, _ = donor_repair_cache[donor["stem"]]
                    save_diagnostic_panel(donor_img_full, donor["bbox"], target_img_full,
                                           target["bbox"], diagnostic, composite,
                                           repaired_donor_cached, panel_path)
                ...
                records.append({
                    ...
                    # Permanent, queryable tier field (Finding 2, point (c)):
                    # 1 = dense full-region search, 2 = dense search on a
                    # shrunk region, 3 = adjacent-space resize or flat fill.
                    # Every repair now has both a tier and a real
                    # selected_score, whichever tier it resolved at, so
                    # 8E can run a sensitivity analysis restricted to
                    # e.g. tier==1 only without needing to regenerate.
                    "repair_tier": diagnostic.get("tier"),
                    "repair_method": diagnostic.get("method"),
                    # Per peer review: a numeric score alone does not make
                    # the constant-color no-texture fallback a legitimate
                    # repair. False for tier3_no_texture_fallback only;
                    # True for every other pathway. Should this ever be
                    # False in the real pilot, that composite must be
                    # pulled for a human decision before it enters B/C,
                    ...
                    # Per peer review: preserve both identifiers so every
                    # composite traces directly to its canonical per-donor
                    # repair artifact (donor_id above + this path).
                    "repaired_donor_path": diagnostic.get("repaired_donor_path"),
    ...
    if scores:
        print(f"\nRepair patch-selection scores (all composites): min={min(scores):.2f}, "
              f"max={max(scores):.2f}, mean={sum(scores)/len(scores):.2f}")
        print("  (Lower = better match to destination surround. Large max values")
        print("   may indicate a composite worth visually spot-checking. Per peer")
        print("   review: score alone does not establish cross-method comparability --")
        print("   pair with tier/method and visual QA, don't threshold on score alone.)")

    # Everything below is reported over UNIQUE donors (not composite
    # records) -- a donor's repair is computed once and reused across
    # every composite that draws on it, so counting per-record would
    # double/triple-count the same repair. This is the real signal for
    # whether the Finding 2 redesign reduced reliance on tier 2/3
    # fallbacks, and it's exactly the breakdown peer review asked the
    # next real-data pilot to report.
    if donor_repair_cache:
        n_donors = len(donor_repair_cache)
        diags = [diag for _, diag in donor_repair_cache.values()]

        tiers = [d.get("tier") for d in diags]
        tier_counts = {t: tiers.count(t) for t in [1, 2, 3]}
        print(f"\nRepair tier distribution across {n_donors} unique donors:")
        for t in [1, 2, 3]:
            c = tier_counts.get(t, 0)
            print(f"  Tier {t}: {c} ({100*c/n_donors:.1f}%)")

        shrink_fracs = [d.get("shrink_fraction_used") for d in diags if d.get("shrink_fraction_used") is not None]
        if shrink_fracs:
            from collections import Counter
            sf_counts = Counter(shrink_fracs)
            print(f"  Tier-2 shrink-fraction distribution (n={len(shrink_fracs)} donors that "
                  f"needed a shrink attempt): " +
                  ", ".join(f"{sf}={sf_counts[sf]}" for sf in sorted(sf_counts, reverse=True)))
            print("    (Smaller shrink fraction = less padding removed around the laser ROI --")
            print("     inspect these first for residual donor-laser halo, per peer review.)")

        no_texture = [d for d in diags if d.get("method") == "tier3_no_texture_fallback"]
        print(f"  tier3_no_texture_fallback occurrences: {len(no_texture)}"
              + (" -- NONE admitted to B/C without review (repair_admissible=False)" if no_texture else ""))
        if no_texture:
            print("  FLAG: this pathway occurred on real data -- per peer review, do not let")
            print("  these composites silently enter the B/C experimental set. Pull them for")
            print("  a human decision before the full sweep.")

        cand_counts = [d.get("candidate_count") for d in diags if d.get("candidate_count") is not None]
        if cand_counts:
            print(f"  Candidate count at the succeeding tier: min={min(cand_counts)}, "
                  f"max={max(cand_counts)}, mean={sum(cand_counts)/len(cand_counts):.1f}")
