"""
05c_training_fft_wavelet.py
===========================
Trains MambaVision_S on the soil moisture laser crop dataset with
...
Run:
    python 05c_training_fft_wavelet.py

Output:
    results/mambavision_fft_wavelet_training_results.json
    results/mambavision_fft_wavelet_training_curves.png
    results/mambavision_fft_wavelet_best_model.pth
"""
...
def extract_fft_channel(img_np):
    ...
    """
    gray = np.mean(img_np, axis=2)                  # (H, W)
    f    = fftshift(fft2(gray))
    mag  = np.log1p(np.abs(f))                      # log-scale magnitude
    mag  = (mag - mag.min()) / (mag.max() - mag.min() + 1e-8)
    return mag.astype(np.float32)                    # (H, W)
...
def extract_wavelet_channels(img_np):
    ...
    for coef in [cH, cV, cD]:
        ...
        # Resize back to original spatial dims via PIL
    ...
    return channels                                  # list of 3 x (H, W)
...
class FFTWaveletDataset(Dataset):
    ...
    def __getitem__(self, idx):
        tensor, label = self.dataset[idx]             # (3, H, W), scalar

        # Convert tensor back to numpy for FFT/Wavelet
        img_np = tensor.permute(1, 2, 0).numpy()      # (H, W, 3), float32

        # FFT channel — (H, W)
        fft_ch = extract_fft_channel(img_np)

        # Wavelet channels — 3 x (H, W)
        wav_chs = extract_wavelet_channels(img_np)

        # Stack all channels: 3 RGB + 1 FFT + 3 Wavelet = 7
        fft_tensor = torch.from_numpy(fft_ch).unsqueeze(0)          # (1, H, W)
        wav_tensor = torch.stack(
            [torch.from_numpy(c) for c in wav_chs], dim=0)          # (3, H, W)

        combined = torch.cat([tensor, fft_tensor, wav_tensor], dim=0) # (7, H, W)
...
# ═════════════════════════════════════════════════════════════════════════════
# 5B. PHYSICAL AUGMENTATION — expand training set 3x (same as 05_training.py)
...
class MambaVisionFFTWavelet(nn.Module):
    ...
    def forward(self, x):
        x = self.input_proj(x)      # (B, 7, H, W) → (B, 3, H, W)
    ...
    if val_acc > best_val_acc:
        ...
        torch.save(model.state_dict(),
                   os.path.join(RESULTS_DIR,
                                "mambavision_fft_wavelet_best_model.pth"))
...
results = {
    "model"                    : "MambaVision_S_FFTWavelet",
    "input_channels"           : 7,
...
output_path = os.path.join(RESULTS_DIR,
                           "mambavision_fft_wavelet_training_results.json")
...
fig.suptitle("MambaVision_S + FFT + Wavelet — Laser Crops Training\n"
...
fig_path = os.path.join(RESULTS_DIR,
                        "mambavision_fft_wavelet_training_curves.png")
plt.savefig(fig_path, dpi=150, bbox_inches="tight")
plt.close()
print(f"Training curves saved → {fig_path}")

print("\nDone! Ready for 05bc_training_fft_wavelet_fullimage.py")