...
Usage:
    python phase8b_evaluate.py --model <name> --weights <path_to.pt> \
        --data <path_to_data.yaml> --split val
"""
...
def evaluate_accuracy(model, data_yaml, split="val"):
...
def main():
    ...
    parser.add_argument("--split", type=str, default="val",
                         help="Which split to evaluate accuracy on: train, val, or test")
    args = parser.parse_args()

    print("=" * 70)
    print(f"  Phase 8B Evaluation — {args.model} (split={args.split})")
    print("=" * 70)

    model = YOLO(args.weights)

    print("Running accuracy evaluation...")
    accuracy_results = evaluate_accuracy(model, args.data, split=args.split)
    ...
    out_path = os.path.join(RESULTS_DIR, f"phase8b_{args.model}_{args.split}_results.json")
